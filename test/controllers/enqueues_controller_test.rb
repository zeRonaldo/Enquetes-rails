...
class EnqueuesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create enqueue" do
    assert_difference('Enqueue.count') do
      post enqueues_url, params: { enqueue: { category: @enqueue.category, text: @enqueue.text, title: @enqueue.title, type: @enqueue.type, user_id: @enqueue.user_id } }
    end
  ...
  test "should update enqueue" do
    patch enqueue_url(@enqueue), params: { enqueue: { category: @enqueue.category, text: @enqueue.text, title: @enqueue.title, type: @enqueue.type, user_id: @enqueue.user_id } }
